...
import './App.css';
import kanjiJouyou from './data/kanji-jouyou-grouped-50.json';
  ...
  return (
    <div className="container-fluid">
      ...
      <div className="float-right">
        <button className="btn-primary btn-lg mt-2" onClick={() => setKanjiState({})}>
          リセット
        </button>
      ...
      <div className="row">
        {kanjiJouyou.map((kanji, index) => (
          <div key={index} className="col-sm-2 mb-3" onClick={() => navigate(`/kanji/${index}`)}>
            <div
              className={`card ${
            ...
            >
              <div className="card-body text-center cursor-pointer">
                ...
                ({index * total + 1} - {index * total + kanji.length + (kanjiJouyou.length === index + 1 ? 1 : 0)})
              </div>
            </div>
          </div>
        ))}
